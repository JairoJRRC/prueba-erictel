...
class Config
{
    ...
    public function establishConnection()
    {
        ...
        if ($conn) {
            $validateTableExist = "SELECT count(*) as 'exist' FROM INFORMATION_SCHEMA.TABLES  WHERE TABLE_NAME = N'TB_USUARIO'";
            $isNullTable = sqlsrv_query($conn, $validateTableExist);
            if (! (bool) sqlsrv_fetch_array($isNullTable, SQLSRV_FETCH_ASSOC)['exist']) {
                $queryCreateTableUser = "CREATE TABLE TB_USUARIO (id INT NOT NULL identity(1,1), name varchar(255), last_name varchar(255), address varchar(255), city varchar(255), PRIMARY KEY (id));";
                sqlsrv_query($conn, $queryCreateTableUser);
